...
public class ObjImporter : MonoBehaviour {
    ...
    private delegate Mesh TextDelegate(string text);
    ...
    public IEnumerator ImportFile(string filePath) {
        //VERY IMPORTANT OR WWW FREAKS \n character doesn't show in debug logs either
        filePath = (filePath.Replace('\\', '/')).Replace("\r", "").Replace("\n", "");

        IEnumerator co = doWWW("file:///" + filePath, ProcessFile); // create an IEnumerator object;

        yield return StartCoroutine(co);
    }

    private Mesh ProcessFile(string textFromFile) { 
		Debug.Log ("creating object");

        meshStruct newMesh = createMeshStruct(textFromFile);

        populateMeshStruct(ref newMesh, textFromFile);
	...
	private meshStruct createMeshStruct(string entireText)
	{
		int triangles = 0;
		int vertices = 0;
		int vt = 0;
	...
	public void DoFileDialogue() {
        ...
        StartCoroutine(GetComponent<ObjImporter>().ImportFile(_path));
